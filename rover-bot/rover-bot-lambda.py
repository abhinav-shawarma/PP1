...
def get_photo(date):
    ...
    api_key = 'DEBUG'
    url = 'https://api.nasa.gov/mars-photos/api/v1/rovers/curiosity/photos?&api_key={}&earth_date={}'.format(api_key, date)
...
def lambda_handler(event, context):
    ...
    while photo is None:
        ...
        logger.debug(
            'Searching for an image taken on [{}]'.format(random_date))

        try:
            photo = get_photo(random_date)
            ...
            if len(img) < 30000:
                ...
                with open("/tmp/rover_image", 'wb') as f:
                    f.write(img)
        except IndexError:
            logger.info(
                'No images for search criteria, trying again in 10s...')
            time.sleep(10)
    ...
    with open("/tmp/rover_image", "rb") as image:
        GraphAPI(access_token=fb_access_token).put_photo(
